...
if ($get_id !== null) {
    ...
    // ... (similar checks for other uses of $get_id in your code)
}
?>
                                        ...
                                        <tr>
                                            ...
                                            <td width="220">
                                                <form id="assign" method="post"
                                                    action="submit_assignment.php<?php echo '?id='.$get_id ?>&<?php echo 'post_id='.$id ?>">
                                                    <input type="hidden" name="id" value="<?php echo $id; ?>">

                                                    <?php if ($floc == "") { ?>
                                                    <!-- File not available, display a message or handle accordingly -->
                                                    No file available
                                                    <?php } else { ?>
                                                    <a data-placement="bottom" title="Download"
                                                        id="<?php echo $id; ?>download" class="btn btn-info"
                                                        href="<?php echo $row['floc']; ?>">
                                                        <i class="icon-download icon-large"></i>
                                                    </a>
                                                    <?php } ?>

                                                    <button data-placement="bottom" title="Submit Assignment"
                                                        id="<?php echo $id; ?>submit" class="btn btn-success"
                                                        name="btn_assign">
                                                        <i class="icon-upload icon-large"></i> Submit Assignment
                                                    </button>
